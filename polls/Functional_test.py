...
class FunctionalTest(TestCase):
    ...
    def test05_editar(self):
        ...
        self.browser.find_element_by_xpath("//select[@id='id_tiposDeServicio']/option[text()='Desarrollador Web']").click()
        telefono = self.browser.find_element_by_id('id_telefono')
        telefono.send_keys('3182762012')
        correo = self.browser.find_element_by_id('id_correo')
        correo.send_keys('cf.agudelo96@gmail.com')
        imagen = self.browser.find_element_by_id('id_imagen')
        imagen.send_keys('C:/Users/cfagu/Documents/GitHub/Kata002/polls/templates/img/IMG_0282.jpg')
        botonGrabar = self.browser.find_element_by_id('id_editar')
        botonGrabar.click()
        self.browser.implicitly_wait(10000)
        span = self.browser.find_element(By.XPATH, '//span[text()="Carlos Agudelo"]')
        self.assertIn("Carlos Agudelo", span.text)